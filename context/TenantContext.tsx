...
const IS_OFFLINE_TEST_MODE = true;
...
export const TenantProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
      ...
      try {
        ...
        if (tenants.length > 0) {
          ...
          if (dbTenant.id.length !== 36) {
            console.error("INVALID TENANT ID DETECTED (Local Only). Destroying...", dbTenant.id);
            await database.write(async () => {
              await dbTenant.markAsDeleted();
              await dbTenant.destroyPermanently();
            });
            setIsOnboarded(false); // Force Re-Login
            return;
          }
    ...
    const subInvoices = invoicesCollection.query()
      ...
      .subscribe((data) => {
        const plainInvoices = data.map(i => ({
          ...
          status: i.status,
          date: i.dateIssued.toISOString().split('T')[0],
          items: i.items,
  ...
  const addInvoice = async (inv: Invoice) => {
    await database.write(async () => {
      ...
      if (inv.status !== 'draft') {
        await database.collections.get<TransactionModel>('transactions').create(newTx => {
          newTx._raw.id = 'tx_' + Date.now() + '_inv';
  ...
  const updateInvoice = async (id: string, updates: Partial<Invoice>) => {
    try {
      await database.write(async () => {
        ...
        // If we just generated a PDF (Evidence), update the transaction
        if (updates.pdfGeneratedAt) {
          const linkedTx = await database.collections.get<TransactionModel>('transactions').query().fetch();
          // Ideally we use Q.where('invoice_id', id) but simplified for MVP w/o setting up index yet
          // Using refId or manual filter
          const tx = linkedTx.find(t => t.refId === id || (t as any).invoiceId === id);
          if (tx) {
            await tx.update(t => {
              t.hasVatEvidence = true; // Mark as Verified
              t.appSyncStatus = 'pending';
            });
          }
        }
  ...
  const performFullPull = async (tenantId: string) => {
    ...
    try {
      ...
      // A. Transactions
      const txRes = await fetch(`${API_BASE}/api/sync/transactions?tenantId=${tenantId}`);
  ...
  const performSyncInternal = async (activeTenantId: string) => {
    ...
    try {
      // 1. Transactions Pull
      const tRes = await fetch(`${API_BASE}/api/sync/transactions?tenantId=${activeTenantId}`);
      if (tRes.ok) {
  ...
  const performSync = async () => {
    ...
    try {
      ...
      // 1. PUSH Transactions
      const pendingTxs = transactions.filter(t => t.syncStatus === 'pending');
      if (pendingTxs.length > 0) {
        await fetch(`${API_BASE}/api/sync/transactions`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ tenantId: tenant.id, transactions: pendingTxs })
        });
        // Optimistic update local
        const txCollection = database.collections.get<TransactionModel>('transactions');
        const batchUpdates = await Promise.all(pendingTxs.map(async (tx) => {
          try {
            const dbTx = await txCollection.find(tx.id);
            return dbTx.prepareUpdate(record => { record.appSyncStatus = 'synced'; });
          } catch (e) { return null }
        }));
        const validUpdates = batchUpdates.filter(Boolean) as any[];
        if (validUpdates.length > 0) await database.batch(validUpdates);
      }

      // 2. PUSH Invoices
      const pendingInvoices = invoices.filter(i => i.syncStatus === 'pending');
      if (pendingInvoices.length > 0) {
        const invRes = await fetch(`${API_BASE}/api/sync/invoices`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ tenantId: tenant.id, invoices: pendingInvoices })
        });
        if (invRes.ok) {
          const invCollection = database.collections.get<InvoiceModel>('invoices');
          const batchUpdates = await Promise.all(pendingInvoices.map(async (inv) => {
            try {
              const dbInv = await invCollection.find(inv.id);
              return dbInv.prepareUpdate(r => { r.appSyncStatus = 'synced' });
            } catch (e) { return null }
          }));
          const valid = batchUpdates.filter(Boolean) as any[];
          if (valid.length > 0) await database.batch(valid);
        }
      }
